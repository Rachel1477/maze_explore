...
class Draw_ui(FigureCanvas):
    ...
    def draw_root(self, my_maze, start_pos, time_, period, fire_flag,first=False):
        ...
        for i, cell in enumerate(my_maze.visited):
            if i + 1 >= time_ and time_ > 0:
                if(first!=True): self.axes_root.plot(cell[1], cell[0], color='purple', marker='*', markersize=int(30 * size_factor))
        ...
        self.axes_root = self.draw_money(self.axes_root, my_maze)
        if fire_flag:
    ...
    def draw_money(self, ax, my_maze):
        size_factor = 7.0 / my_maze.maze.shape[0]
        for i, cell in enumerate(my_maze.money):
            ax.plot(cell[1], cell[0], color='gold', marker='o', markersize=int(38 * size_factor))
            ax.text(cell[1], cell[0], "coin", ha="center", va="center", color="black", fontsize=int(10*size_factor))
        return ax
