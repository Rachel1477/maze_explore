...
def draw_qtable(ax, qtable_model, time_, fire_flag,algorithm_choice):
    ...
    ax = draw_money(ax, qtable_model.my_maze.maze)
    if fire_flag:
...
def draw_root(ax, my_maze, start_pos, time_, period, fire_flag):
    ...
    for i, cell in enumerate(my_maze.visited):
        if i+1>=time_ and time_>0:
            ax.plot(cell[1], cell[0], color='purple', marker='*', markersize=int(30 * size_factor))
...
def draw_trap(ax, my_maze):
    ...
    return ax
def draw_money(ax,my_maze):
    size_factor = 7.0 / my_maze.maze.shape[0]
    for i, cell in enumerate(my_maze.money):
        ax.plot(cell[1], cell[0], color='gold', marker='o', markersize=int(38 * size_factor))
        ax.text(cell[1], cell[0], "coin", ha="center", va="center", color="black")
    return ax
...
def draw_table_root_byTime(qtable_model, my_maze, start_pos):
    ...
    while True:
        ...
        draw_qtable(ax1, qtable_model, time_%period if period!=0 else 0, fire_flag,my_maze.al)
